...
def main():
    ...
    host_info = get_response(
        'analyze',
        host='www1.firstdirect.com',
        fromCache='on',
        all='done')
    pprint(host_info)
...
class SSLTest(object):
    ...
    def __init__(self, host, use_cache=True):
        params = dict(
            fromCache='on',
            all='done') if use_cache else dict(
            startNew='on',
            all='done')
        self._host_info = get_response('analyze', host=host, **params)
        ip_addr = self._host_info['endpoints'][0]['ipAddress']
        self._endpoint_info = get_response(
            'getEndpointData',
            host=host,
            fromCache='on' if use_cache else '',
            s=ip_addr)
        self.grade = self._endpoint_info['grade']
        self.drown_vulnerable = self._endpoint_info[
